# Plan: PWNGHOST-RS Implementation
...
│       │                                                                       │
│       ▼                                                                       │
│  pi-gen stage3 (kernel + nexmon + firmware) ◄──────────────┐                 │
│       │                                                     │                 │
│       ▼                                                     │                 │
│  pi-gen stage4 (Rust toolchain + cross-compile)  fw-patcher crate          │
│       │                                                     │                 │
│       ▼                                                     ▼                 │
│  pi-gen stage5 (install artifacts) ──────────────► angryoxide crate         │
│       │                                                     │                 │
│       ▼                                                     ▼                 │
│                                                         pwncore crate         │
│                                                           │                 │
...
| M1.6 | `fw-patcher`: SDIO RAMRW crash counter monitoring (from oxigotchi) | M1.1 | Reads crash counters via netlink |
| M1.7 | `angryoxide`: subprocess spawn + crash detection | M0.3 | Spawns, captures stdout |
| M1.8 | `angryoxide`: JSON line parser (AP, handshake, stats events) | M1.7 | Parses real AO output |
| M1.9 | `angryoxide`: CLI arg builder from config | M1.8 | Generates valid AO args |
| M1.10 | `angryoxide`: exponential backoff recovery | M1.7 | Restarts on crash |
...
| Task | Crate/Component | Dependencies | Verification |
|------|----------------|--------------|--------------|
| M3.1 | `agent`: epoch.rs (tracking, mood transitions) | M2.2, M1.8 | Mood triggers match personality.toml |
...
| M3.4 | `agent`: agent.rs (FSM: Recon→Attack→Hop) | M1.8, M3.1 | Runs 3 epochs vs mock AO |
...
| Task | Crate/Component | Dependencies | Verification |
|------|----------------|--------------|--------------|
| M4.1 | `agent`: healing.rs (6-layer state machine) | M1.4, M1.10 | Triggers on injected failures |
| M4.2 | `agent`: capture.rs (tmpfs → .22000 + .pcapng) | M1.8 | Validates handshake quality |
| M4.3 | Integration: healing + AO watchdog + GPIO | M4.1, M1.10 | Full crash→recovery cycle |
...
| Task | Crate/Component | Dependencies | Verification |
|------|----------------|--------------|--------------|
| M9.1 | `pwnghost-rs`: main.rs (wiring all crates) | All prior | Binary runs |
| M9.2 | `pwnghost-rs`: boot.rs (first-boot fw patch) | M1.5 | Applies patch on boot |
...
| **WS-D: AngryOxide Wrapper** | Backend | angryoxide | M0.3 | M1.6-M1.10 |
...
| **WS-K: Healing/Capture** | Backend | agent/healing, capture | M1.10, M3.4 | M4.1-M4.3 |
