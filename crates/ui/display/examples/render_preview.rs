...
fn main() {
    ...
    let faces = [("(◕‿‿◕)", "awake.png"), ("(-_-')", "angry.png"), ("(⌐■_■)", "cool.png"), ("(♥‿‿♥)", "friend.png")];
    for (face, name) in faces {
        for b in buf.iter_mut() { *b = 0; }
        engine.draw_pwnagotchi_frame(&mut buf, w, h, 6, 3, "1234s", "pwnghost", "This is working!", face, 2, 7, 3, 150, "AUTO", None).unwrap();
        // Encode 1bpp buf -> grayscale PNG (bit=1 => black ink)
        let mut img = vec![255u8; (w*h) as usize];
        for y in 0..h { for x in 0..w {
            let idx = (y*w + x) as usize;
            if (buf[idx/8] >> (idx%8)) & 1 != 0 { img[idx] = 0; }
        }}
